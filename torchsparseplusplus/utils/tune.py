...
from torchsparseplusplus.nn import Conv3d
...
def torchsparseplusplus_tune_timer(
    model: nn.Module,
    inputs: SparseTensor,
    tune_with_bwd: bool,
) -> float:
...
def dataflow_selector(
    ...
    for group_idx, names in group_to_name.items():
        ...
        if F.Dataflow.ImplicitGEMM in dataflow_range:
            ...
            fwd_duration, bwd_duration = torchsparseplusplus_tune_timer(
...
def tune(
    ...
    save_dir: str = ".torchsparseplusplus-tune",
    tune_tag: str = "temp",
    ...
    if not torchsparseplusplus.backends.benchmark:  # type: ignore
        if not skip_warning:
            print(
                "Warning: to use tuning, "
                + "torchsparseplusplus.backends.benchmark is automatically set to be true."
            )
        torchsparseplusplus.backends.benchmark = True  # type: ignore
