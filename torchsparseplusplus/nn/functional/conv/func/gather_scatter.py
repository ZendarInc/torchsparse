...
class GatherScatterConvolutionFuntion(Function):  # torchsparseplusplus_v2
    @staticmethod
    ...
    def forward(
        ...
        conv_mode = 0
        global buffer
        if torchsparseplusplus.backends.benchmark:  # type: ignore
        ...
        if input.device.type == "cuda":
            ...
            output = torchsparseplusplus.backend.conv_forward_gather_scatter_cuda(
                input,
                weight,
                nbmaps,
                nbsizes.cpu(),
            ...
            torchsparseplusplus.backend.conv_forward_gather_scatter_cpu(
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if grad_output.device.type == "cuda":
            torchsparseplusplus.backend.conv_backward_gather_scatter_cuda(
                input,
                grad_input,
            ...
            torchsparseplusplus.backend.conv_backward_gather_scatter_cpu(
                input,
                grad_input,
                grad_output.contiguous(),
                weight,
