...
class ImplicitGEMMConvolutionFuntion(Function):  # torchsparseplusplus++
    @staticmethod
    ...
    def forward(
        ...
        if input.device.type == "cuda":
            ...
            if not ifsort:
                output = torchsparseplusplus.backend.conv_forward_implicit_gemm_cuda(
                    input,
                    weight,
                    out_in_map,
                    num_out_feats,
                    num_out_channels,
                    torchsparseplusplus.backends.allow_tf32,
                    torchsparseplusplus.backends.allow_fp16,
                )
            else:
                output = torchsparseplusplus.backend.conv_forward_implicit_gemm_sorted_cuda(
                    input,
                    weight,
                    reorder_out_in_map,
                    reduced_sorted_mask,
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if grad_output.device.type == "cuda":
            if kernel_volume < 32:  # sort mode
                # dgrad
                grad_input = torchsparseplusplus.backend.conv_forward_implicit_gemm_sorted_cuda(
                    grad_output,
                ...
                grad_weight = (
                    (
                        torchsparseplusplus.backend.conv_backward_wgrad_implicit_gemm_sorted_cuda(
                            grad_output,
                            input,
                            reorder_out_in_map_bwd,
                            reduced_sorted_mask_bwd_wgrad,
                ...
                # dgrad
                grad_input = torchsparseplusplus.backend.conv_forward_implicit_gemm_cuda(
                    grad_output,
                    weight.transpose(2, 1).contiguous(),
                    out_in_map_bwd,
                    input.size(0),
                ...
                grad_weight = (
                    (
                        torchsparseplusplus.backend.conv_backward_wgrad_implicit_gemm_cuda(
                            grad_output,
                            input,
                            out_in_map_bwd,
                            32,
