...
def sphashquery(queries: torch.Tensor, references: torch.Tensor) -> torch.Tensor:
    ...
    if queries.device.type == "cuda":
        hashtable = torchsparseplusplus.backend.GPUHashTable(references.shape[0] * 2)
        hashtable.insert_vals(references)
        output = hashtable.lookup_vals(queries)
    elif queries.device.type == "cpu":
        output = torchsparseplusplus.backend.hash_query_cpu(queries, references, indices)
    else:
        device = queries.device
        output = torchsparseplusplus.backend.hash_query_cpu(
...
def convert_transposed_out_in_map(out_in_map, size):
    ...
    torchsparseplusplus.backend.convert_transposed_out_in_map(out_in_map, out_in_map_t)
    return out_in_map_t
