...
def sphash(
    ...
    if offsets is None:
        if coords.device.type == "cuda":
            return torchsparseplusplus.backend.hash_cuda(coords)
        elif coords.device.type == "cpu":
            return torchsparseplusplus.backend.hash_cpu(coords)
        else:
            device = coords.device
            return torchsparseplusplus.backend.hash_cpu(coords.cpu()).to(device)
    else:
        ...
        if coords.device.type == "cuda":
            return torchsparseplusplus.backend.kernel_hash_cuda(coords, offsets)
        elif coords.device.type == "cpu":
            return torchsparseplusplus.backend.kernel_hash_cpu(coords, offsets)
        else:
            device = coords.device
            return torchsparseplusplus.backend.kernel_hash_cpu(coords.cpu(), offsets.cpu()).to(
                device
            )
