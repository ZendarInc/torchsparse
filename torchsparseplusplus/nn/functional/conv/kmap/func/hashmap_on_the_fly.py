...
import torchsparseplusplus.backends
from torchsparseplusplus.utils import make_tensor
...
def build_kmap_implicit_GEMM_hashmap_on_the_fly(
    ...
    if torchsparseplusplus.tensor.get_allow_negative_coordinates():
    ...
    if subm:
        func = torchsparseplusplus.backend.build_kernel_map_subm_hashmap
    else:
        func = torchsparseplusplus.backend.build_kernel_map_downsample_hashmap
    to_insert = False

    assert (
        torchsparseplusplus.backends.hash_rsv_ratio >= 2
    ), f"hash_rsv_ratio should be no less than 2, now {torchsparseplusplus.backends.hash_rsv_ratio}."
    hashmap_capacity = max(
        512, int(torchsparseplusplus.backends.hash_rsv_ratio * _coords.shape[0])
    )
    ...
    hashtable = torchsparseplusplus.backend.GPUHashTable(
    ...
    if ifsort:
        ...
        reorder_out_in_map = torchsparseplusplus.backend.reorder_out_in_map_cuda(
            out_in_map, reorder_loc
        )
        reduced_sorted_mask = torchsparseplusplus.backend.reduce_bitmask_cuda(
            sorted_mask, cta_M
        )
...
def build_kmap_Gather_Scatter_hashmap_on_the_fly(
    ...
    input_mask, output_mask = torchsparseplusplus.backend.build_mask_from_kmap(
        _coords.shape[0],
        kmap["coords"].shape[0],
        nbmaps.int(),
...
def build_kmap_Fetch_on_Demand_hashmap_on_the_fly(
    ...
    torchsparseplusplus.backend.exclusive_scan_quantified_wrapper(
